...
import { Search, ShoppingCart, CheckCircle2, ChevronRight, X, AlertCircle, UserCog, Plus, Minus, ArrowLeft, MessageCircle, Banknote, Package, QrCode, Volume2, Printer, Smartphone, CreditCard, Zap, Receipt } from 'lucide-react';
...
function VenderContenido() {
  ...
  const ejecutarVentaFinal = async () => {
    ...
    try {
      ...
      const subtotalCalculado = ivaHabilitado ? Math.round(montoAcumulado / (1 + tasaIva)) : montoAcumulado;
      const ivaCalculado = ivaHabilitado ? (montoAcumulado - subtotalCalculado) : 0;
...
*¡Te esperamos pronto!*`;
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white dark:bg-[#0f172a] md:rounded-[2rem] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl min-h-0 animate-in fade-in duration-300">
      ...
      {/* CABECERA */}
      <div className="bg-emerald-600 dark:bg-emerald-700 p-4 md:p-5 text-white flex justify-between items-center shrink-0 z-30 shadow-sm">
        ...
        <button 
          ...
          className="bg-white text-emerald-700 hover:bg-emerald-50 px-4 py-2.5 rounded-xl font-black text-sm flex items-center gap-2 shadow-md transition-transform active:scale-95 cursor-pointer"
        >
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto space-y-4">
              ...
              <div className="space-y-4">
                {filasRegistro.map((fila, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col md:flex-row gap-3 md:gap-4 p-4 md:p-5 bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200 dark:border-slate-800 relative shadow-sm transition-colors hover:border-emerald-300">
                      ...
                      <div className="flex-1 min-w-0 relative">
                        ...
                        <input 
                          ...
                          className="w-full p-3 md:p-4 h-[50px] md:h-[56px] bg-slate-50 dark:bg-[#020617] border border-slate-200 dark:border-slate-800 rounded-xl outline-none font-bold text-base md:text-lg min-w-0 shadow-sm focus:border-emerald-500 transition-colors text-slate-900 dark:!text-white placeholder-slate-400 dark:placeholder-slate-500" 
                        />
                ...
                <button onClick={agregarFila} className="font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 px-5 py-3 md:py-4 rounded-xl transition-colors flex items-center justify-center sm:justify-start gap-2 text-sm shadow-sm w-full sm:w-auto">
        ...
        <div className="w-full lg:w-[380px] xl:w-[420px] bg-white dark:bg-[#0f172a] lg:border-l border-slate-200 dark:border-slate-800 flex flex-col shrink-0 min-h-0 overflow-hidden">
          ...
          {/* Contenedor desplazable de formulario */}
          <div className="flex-1 overflow-y-auto p-4 lg:p-5 space-y-4 min-h-0">
            ...
            <div className="flex flex-col bg-slate-50 dark:bg-[#020617] p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80 shadow-sm relative">
              ...
              {clienteTransaccion ? (
                <div className="py-2.5 px-3.5 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-200 dark:border-emerald-800/50 flex justify-between items-center shadow-sm">
                  <span className="font-black text-slate-900 dark:text-emerald-300 text-sm truncate mr-2">{clienteTransaccion.nombre}</span>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-2 py-3 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold outline-none focus:border-emerald-500 transition-colors shadow-sm text-slate-900 dark:!text-white placeholder-slate-400 dark:placeholder-slate-500"
                  />
            ...
            <div className="flex flex-col bg-slate-50 dark:bg-[#020617] p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80 shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <Smartphone size={15} /> Nequi / Transf.
                </button>
                ...
                >
                  <Zap size={15} /> Addi / Sistecrédito
                </button>
              </div>

              {/* CAMPO DE REFERENCIA CUANDO NO ES EFECTIVO */}
              {metodoPago !== 'efectivo' && (
                <div className="pt-1 animate-in fade-in duration-200">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 block">
                    {metodoPago === 'transferencia' && 'No. de Comprobante / Nequi (Opcional)'}
                    {metodoPago === 'datafono' && 'No. de Voucher / Últimos 4 dígitos (Opcional)'}
                    {metodoPago === 'credito_externo' && 'No. de Aprobación / Cédula (Opcional)'}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={
                      metodoPago === 'transferencia' ? 'Ej: Nequi M123456' : (metodoPago === 'datafono' ? 'Ej: Voucher 9876' : 'Ej: Aprobación 54321')
                    }
                    className="w-full p-2.5 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl outline-none font-bold text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:border-blue-500"
                  />
                </div>
              )}
            </div>

            {/* MONTO RECIBIDO Y VUELTAS */}
            <div className="flex flex-col bg-slate-50 dark:bg-[#020617] p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80 shadow-sm">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <Banknote size={14}/> {metodoPago === 'efectivo' ? 'Dinero entregado' : 'Monto Pagado'}
              </label>
              <div className="relative rounded-xl shadow-sm mb-2">
                ...
                <input type="text" inputMode="numeric" value={pagoCliente} onChange={(e) => setPagoCliente(formatearMonedaInput(e.target.value))} placeholder="Monto..." className="w-full pl-8 pr-2 py-3 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl outline-none font-black text-xl min-w-0 focus:border-emerald-500 transition-colors text-slate-900 dark:!text-white placeholder-slate-400 dark:placeholder-slate-500" />
              </div>
            ...
            {pagoCliente && parseFloat(pagoCliente.replace(/\D/g, '')) >= totalFilasRegistro && totalFilasRegistro > 0 && (
              <div className="p-3.5 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 rounded-xl flex justify-between items-center animate-in zoom-in-95 duration-200 shadow-sm">
                <span className="text-[10px] lg:text-xs uppercase font-bold tracking-wider">Devuelta:</span>
                <span className="text-lg lg:text-xl font-black">${(parseFloat(pagoCliente.replace(/\D/g, '')) - totalFilasRegistro).toLocaleString('es-CO')}</span>
              </div>
            )}
            {pagoCliente !== "" && parseFloat(pagoCliente.replace(/\D/g, '')) < totalFilasRegistro && totalFilasRegistro > 0 && (
              <div className="p-3.5 bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400 rounded-xl flex justify-between items-center animate-in zoom-in-95 duration-200 shadow-sm">
                <span className="text-[10px] lg:text-xs uppercase font-bold tracking-wider">Saldo a Fiar:</span>
                <span className="text-lg lg:text-xl font-black">${(totalFilasRegistro - parseFloat(pagoCliente.replace(/\D/g, ''))).toLocaleString('es-CO')}</span>
              </div>
            )}
          </div>
          ...
          <div className="hidden lg:flex flex-col bg-slate-900 dark:bg-black text-white p-5 shrink-0 border-t border-slate-800 z-30">
            {datosSesion?.habilitarIva && totalFilasRegistro > 0 && (
              <div className="text-xs text-slate-400 mb-2 space-y-0.5 border-b border-slate-800 pb-2">
            ...
            <button onClick={procesarRegistro} className="w-full bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white font-black text-lg py-3.5 rounded-2xl shadow-lg shadow-emerald-900/30 flex justify-center items-center gap-2 transition-all cursor-pointer">
              <span>Vender</span> <CheckCircle2 size={20}/>
            </button>
          </div>
