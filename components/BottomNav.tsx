      ...
      <div className="max-w-md mx-auto flex items-center justify-around px-1 py-1">
        ...
        {puedeGestionarSepares && (
          <Link 
            ...
            className={`flex-1 py-2.5 flex flex-col items-center gap-1 transition-all active:scale-90 relative ${pathname?.startsWith('/dashboard/separe') ? 'text-violet-600 dark:text-violet-400 font-black' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 font-bold'}`}
          >
            <div className="relative">
              <Bookmark size={21} className={pathname?.startsWith('/dashboard/separe') ? 'stroke-[2.5]' : 'stroke-2'} />
              {separesActivosCount > 0 && (
                <span className="absolute -top-1 -right-2 w-3.5 h-3.5 bg-violet-500 text-white text-[8px] font-black rounded-full flex items-center justify-center">
                  {separesActivosCount}
                </span>
              )}
            </div>
            <span className="text-[10px] uppercase tracking-wider">Separe</span>
          </Link>
        )}
        
        {/* Reportes — Solo si tiene permiso y no hay Separe (evita saturar la barra) */}
